...
Todo límite de cálculo se resuelve por **sustitución directa en la rama correspondiente**. Si el ejercicio viene de un gráfico, solo aparecen **saltos finitos**, **huecos** o **puntos continuos** — nunca ramas que se disparan al infinito.
...
7. **Texto de la opción "No existe"** cuando el bilateral no coincide: usar **exactamente** `"No existe"`. Aparece solo cuando corresponde (bilateral con ramas distintas); no rellenar como distractor automático en ejercicios donde el bilateral sí existe.
...
- **Nunca** usar $x \to a^-$ con $a$ negativo en un ejercicio introductorio (confunde el superíndice con el signo). Reservar esos casos para ejercicios avanzados de sub-familia A donde justamente se pregunta por la distinción.
- **Regla crítica 24 (nueva)**: ningún ejercicio de LEXI se enmarca respecto de otro ejercicio de la sesión ("un caso más exigente"), y todo ejercicio abstracto sitúa el concepto (`límite`, `límite lateral`) desde la primera oración, variando la redacción de apertura ejercicio a ejercicio.
...
- **Bilateral existente con "No existe" como distractor**: en ejercicios donde $\lim^- = \lim^+$, elegir "No existe" por reflejo. Verificar que las ramas coincidan antes de descartar.
...
- **"No existe" como opción** solo cuando corresponde (bilateral con ramas distintas). En ejercicios de un lateral solo (sub-A) o de bilateral existente, no forzar "No existe" como distractor.
- **Funciones a trozos** con 2 ramas por defecto; 3 ramas solo si el ejercicio es explícitamente de sub-B avanzada. Punto de quiebre único.
...
Corrección puntual del usuario sobre ejercicios de prueba de este topic (`correciones_analisis_limites_laterales_1.md`), aplicar al regenerar:
...
- **`LEXI_03`**: el enunciado abría con "Un caso más exigente aparece cuando el punto de tendencia ya es negativo, como en...", implicando comparación con un ejercicio anterior y sin situar que se hablaba de límites laterales antes del tecnicismo. Motivó la **regla crítica 24** (nueva en `authoring-context.md`): ningún ejercicio se enmarca respecto de otro de la sesión, y todo ejercicio abstracto sitúa el concepto desde la primera oración.
- **`LEXI_09`**: 
...
- **`GRAF_08`**: sin cambios, queda como **ejemplo modelo** de desarrollo vertical con `aligned` para lateral+bilateral, contrastar contra `LEXI_09` al revisar el resto de los ejercicios con desarrollo de 2+ pasos.
- **Ronda 2 (auditoría de `continuity`)**: se detectó que 4 ejercicios de este topic (`GRAF.json` ×2, `LEXI.json` ×1, `RESL.json` ×1) usaban `\lim^-`/`\lim^+` sin el punto de tendencia en el subíndice. **Ya corregido con un script** (regla crítica 27, nueva en `authoring-context.md`), no volver a introducirlo al generar el resto de los ejercicios.
...
- [ ] **Ningún ejercicio se enmarca respecto de otro de la sesión** (regla crítica 24): sin "un caso más exigente", "a diferencia del anterior"
- [ ] **Todo límite lateral lleva el punto de tendencia en el subíndice** (`\lim_{x \to a^-}`/`\lim_{x \to a^+}`, regla crítica 27); ya corregido en esta ronda (10 ejercicios, ver hallazgos de `continuity`), no reintroducirlo al generar los ejercicios que faltan
- [ ] **Todo ejercicio abstracto (LEXI sin contexto cotidiano) sitúa el concepto desde la primera oración** (regla crítica 24), con redacción de apertura variada ejercicio a ejercicio
