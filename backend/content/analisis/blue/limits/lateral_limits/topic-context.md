...
- **No operabilidad de los laterales**: negar que se pueda calcular $\lim^- + \lim^+$ cuando el bilateral no existe. Los laterales por separado son números reales operables.
...
- **Signo invertido en la suma**: al calcular $\lim^- + \lim^+$, tomar uno de los dos con signo cambiado.
...
- **Explicaciones con `\begin{aligned}`** para el desarrollo paso a paso, mostrando la elección de rama antes de sustituir.
