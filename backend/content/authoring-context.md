...
24. **Nunca enmarcar un ítem en relación a otro ítem de la sesión, y todo ítem abstracto sitúa el concepto desde la primera oración.** Dos partes de la misma regla:
    - **Sin continuidad entre ítems**: prohibido abrir con lenguaje que implique orden o comparación con otro ejercicio ("un caso más exigente", "a diferencia del anterior", "ahora un ejemplo más difícil"). Los ítems se presentan desordenados e independientes en una sesión, no hay garantía de que el alumno haya visto "el anterior". Cada ítem se redacta como si fuera el único que ve.
    - **Situar el concepto antes de ir al tecnicismo**: esto aplica a ítems **abstractos**, sin contexto cotidiano ni gráfico (típicamente `LEXI` puramente conceptual). La primera oración tiene que nombrar de qué objeto matemático se habla (función, límite, límite lateral, derivada, integral, según el belt), antes o junto con el planteo técnico, no asumir que el lector ya sabe qué se está estudiando. **No aplica** a ítems que ya sitúan el concepto por otro medio: `RESL` con una función a trozos explícita, o `GRAF` con un gráfico embebido, ya dan ese contexto por sí solos. **Variar la redacción de apertura ítem a ítem**, no repetir la misma fórmula de frase en toda una sub-familia (se lee como plantilla).
    - ❌ `Un caso más exigente aparece cuando el punto de tendencia ya es negativo, como en\n$$\lim_{x \to (-2)^+} f(x)$$\n¿Qué valores toma $x$ al acercarse de esta manera?` (compara con un ítem implícito anterior, y no sitúa que se está hablando de límites laterales antes del tecnicismo).
    - ✅ `Al trabajar con límites laterales, el punto de tendencia también puede ser negativo, como en\n$$\lim_{x \to (-2)^+} f(x)$$\n¿Qué valores toma $x$ al acercarse de esta manera?` (sitúa "límites laterales" desde la primera oración, sin referirse a otro ítem).
