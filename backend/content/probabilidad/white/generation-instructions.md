...
1. **¿Refactor o generación limpia?** Hoy (ronda 1) todos los `SKILL.json` de `probabilidad` tienen un único ítem dummy: la ronda 1 es generación limpia hasta el target, no refactor de contenido real preexistente.
2. **¿Para qué skill/s?** (`LEXI`, `CLSF`, `FORM`, `ESTR`, `RESL`, etc.)
3. **¿Restricción especial para esta pasada?** (ej. "5 ítems para testing primero", "solo el bloque de Laplace")
...
Antes de generar cualquier `question`, **escribí el plan numerado completo en el chat**: una línea por ítem con el concepto/sub-familia asignado y el contador de cupo, igual que en `analisis` (ver ejemplo en cualquier `generation-prompt.md` de ese curso). Reglas del planning:
...
- Los contextos/ítems obligatorios marcados como tal en el `topic-context.md` aparecen textualmente en el plan.
...
Escribí los ítems en el orden del plan, respetando la clasificación asignada. Aplicá `authoring-context.md` y `gamification-context.md` sin excepciones (ver checklist más abajo, referencia por número de regla).
...
`SKILL_decisions.md` con una entrada por ítem, mismo formato que en `analisis` (ver `analisis/white/generation-instructions.md` sección *Output format* para la plantilla exacta): tabla de plan cumplido, una entrada por ítem con concepto/contador/contexto/distractores/cardinalidad, y tabla de **conteo real** revisado tras generar (obligatoria, aunque coincida con el plan).
...
- [ ] Cada ítem lleva `"tags": ["<slug>"]` con el slug de su fila en la tabla de distribución de su skill (`authoring-context.md` sección *Etiquetas*); contá por slug y verificá contra el target
- [ ] Ítems/contextos obligatorios del `topic-context.md` están presentes, citados textual, no sustituidos por un contexto "equivalente"
...
- [ ] Regla 9, 24 (32 en `analisis`, aplica igual): ninguna oración se corta a la mitad para insertar un `$$...$$`; la apertura antes de un display no se repite literalmente ítem tras ítem
...
- [ ] Regla 24: ningún ítem se enmarca en relación a otro ítem de la sesión ("un caso más exigente", "a diferencia del anterior"); la primera oración nombra la estructura con la que se trabaja ("la expresión", "el evento", "la probabilidad") sin explicarla ni definirla antes de plantear el problema
...
- [ ] Regla 31: cada ítem reintroduce la fórmula/definición central que usa (ej. la fórmula de Bayes, la regla de Laplace), sin asumir que el alumno la vio en otro ítem de la sesión
...
1. **NUNCA generes un ítem sin planning previo.**
2. **NUNCA excedas el cupo de una sub-familia**, aunque tengas una idea buena.
3. **NUNCA sustituyas un ítem/contexto pedido textualmente** en `topic-context.md` por un equivalente. O lo generás textual o lo marcás `[PENDIENTE]`.
...
27. **NUNCA enmarques un ítem en relación a otro ítem de la sesión**, y nombrá la estructura en la primera oración sin explicarla (regla crítica 24 de `authoring-context.md`).
28. **Cada ítem reintroduce la fórmula/definición central que usa** (regla crítica 31).
