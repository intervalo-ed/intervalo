...
export const catalog: Catalog = {
  ...
  "belts": [
    ...
    {
      "key": "violet",
      "headline": "Espacios vectoriales",
      "description": "Un **espacio vectorial** es una estructura algebraica que formaliza y generaliza las propiedades de los vectores, permitiendo estudiar la independencia lineal, los sistemas de generadores, las bases y la noción de dimensión.\n$$\\alpha\\vec{u} + \\vec{v} \\in S$$\nEsta unidad cubre subespacios vectoriales, combinaciones lineales, dependencia lineal, conjuntos generadores, bases y dimensiones de subespacios.",
      "units": [
        {
          "key": "spaces",
          "name": "Espacios",
    ...
    {
      "key": "brown",
      "headline": "Transformaciones lineales",
      "description": "Una **transformación lineal** es una función entre espacios vectoriales que preserva las operaciones esenciales de suma y multiplicación escalar, mapeando rectas en rectas y manteniendo el origen inalterado.\n$$T(\\alpha\\vec{u} + \\vec{v}) = \\alpha T(\\vec{u}) + T(\\vec{v})$$\nEsta unidad cubre la definición, el núcleo (kernel), la imagen, el teorema de la dimensión y la matriz asociada a la transformación.",
      "units": [
        {
          "key": "transformations",
          "name": "Transformaciones",
          "description": "Una transformación lineal mapea vectores de un dominio a un codominio preservando la linealidad.\n$$T(\\alpha\\vec{u} + \\vec{v}) = \\alpha T(\\vec{u}) + T(\\vec{v})$$\nEsta unidad profundiza en el comportamiento algebraico y la representación matricial de estas funciones vectoriales.",
          "topics": [
            {
              "key": "definition",
              "name": "Definición",
              "tooltip": "Una **transformación lineal** es una función entre espacios vectoriales que preserva las operaciones de suma y multiplicación por escalar.\n$$T(\\alpha\\vec{u} + \\vec{v}) = \\alpha T(\\vec{u}) + T(\\vec{v})$$\nGarantiza que la estructura geométrica se mantenga: rectas siguen siendo rectas y el origen no varía. Modela rotaciones, proyecciones y reflexiones.",
              "short_description": "Una **transformación lineal** es una función entre espacios que preserva la suma y el producto escalar.\n$$T(\\alpha\\vec{u} + \\vec{v}) = \\alpha T(\\vec{u}) + T(\\vec{v})$$",
              "skills": [
                "LEXI",
                "CLSF"
              ]
            },
            {
              "key": "kernel",
              "name": "Núcleo",
              "tooltip": "El **núcleo** o *kernel* de una transformación es el conjunto de todos los vectores del dominio que son mapeados al vector nulo.\n$$\\ker(T) = \\{ \\vec{v} \\in V \\mid T(\\vec{v}) = \\vec{0} \\}$$\nEs un subespacio que mide la pérdida de información algebraico-geométrica. Si contiene solo al vector nulo, la transformación es inyectiva.",
              "short_description": "El **núcleo** o *kernel* es el conjunto de vectores del dominio que se mapean al origen de llegada.\n$$\\ker(T) = \\{ \\vec{v} \\in V \\mid T(\\vec{v}) = \\vec{0} \\}$$",
              "skills": [
                "LEXI",
                "CLSF",
                "RESL"
            ...
            },
            {
              "key": "image",
              "name": "Imagen",
              "tooltip": "La **imagen** de una transformación es el conjunto de todos los vectores que pueden alcanzarse en el espacio de llegada.\n$$\\text{Im}(T) = \\{ T(\\vec{v}) \\mid \\vec{v} \\in V \\}$$\nCoincide con el subespacio generado por las columnas de su matriz asociada. Si cubre todo el codominio, la transformación es sobreyectiva.",
              "short_description": "La **imagen** de una transformación es el conjunto de todos los vectores alcanzados en el codominio.\n$$\\text{Im}(T) = \\{ T(\\vec{v}) \\mid \\vec{v} \\in V \\}$$",
              "skills": [
                "LEXI",
                "CLSF",
                "RESL"
            ...
            },
            {
              "key": "theorem",
              "name": "Teorema",
              "tooltip": "El **teorema de la dimensión** establece que la dimensión del dominio se conserva y se reparte entre la imagen y el núcleo.\n$$\\dim(\\ker(T)) + \\dim(\\text{Im}(T)) = \\dim(V)$$\nCada dimensión del dominio que colapsa y desaparece en el núcleo se resta directamente de las dimensiones efectivas que tendrá la imagen final.",
              "short_description": "Establece que la dimensión del dominio equivale a la suma de la dimensión del núcleo y de la imagen.\n$$\\dim(\\ker(T)) + \\dim(\\text{Im}(T)) = \\dim(V)$$",
              "skills": [
                "LEXI",
                "ESTR",
                "RESL"
            ...
            {
              "key": "associated_matrix",
              "name": "Asociada",
              "tooltip": "Toda transformación lineal entre espacios de dimensión finita puede representarse multiplicando un vector de coordenadas por una **matriz asociada**.\n$$T(\\vec{x}) = A\\vec{x}$$\nEsta matriz actúa como diccionario algebraico, calculando el efecto exacto sobre cualquier punto analizando los elementos de la base original.",
              "short_description": "Toda transformación lineal de dimensión finita puede expresarse mediante una **matriz asociada**.\n$$T(\\vec{x}) = A\\vec{x}$$",
              "skills": [
                "LEXI",
                "FORM",
                "RESL"
