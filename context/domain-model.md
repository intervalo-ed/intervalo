...
| `GamePlayer` (`game_players`) | La ficha del jugador del minijuego, **con o sin cuenta**: `user_id` en NULL es un invitado, y son la mayoría. Guarda el Elo (`theta`, `n_updates`), el progreso (`xp`, combos, `exercises_correct`), quién lo trajo (`referred_by`) y la atribución de primer contacto (`platform`, `timezone`, `first_group_id`, `first_utm_source`), que se completa una sola vez y no se pisa. **`timezone`** es el huso que reporta el navegador y decide cuánto le sale un cafecito a esa persona (`boosts.PRECIO_POR_PAIS`); no confundirlo con `notify_timezone`, de la misma tabla, que es dónde está parada hoy y sí cambia porque decide a qué hora real le llega el recordatorio. **`variant`** es el brazo del experimento en curso, como `<experimento>:<brazo>`, y se escribe SOLO al crear la fila (`game/router.py :: _anotar_variante`) — a diferencia de la atribución, que se puede completar en una visita posterior sin mentir, un brazo puesto más tarde sería enrolar a alguien que ya vio la pantalla del control. |
| `GameAttempt` (`game_attempts`) | Un intento de una derivada. `xp_awarded` es lo que se pagó **ya multiplicado**, y `xp_from_boost` cuánto de eso lo puso el empuje de cafecito — gemela de `Answer.xp_from_boost` y por el mismo motivo: el multiplicador de ese momento no queda en ninguna fila, así que el reparto no se puede reconstruir después. Es lo que permite preguntar cuánto generó el empuje de una universidad en una ventana de horas (`game/boosts.py :: efecto_del_empuje`); el acumulado `game_players.xp_from_boosts` guarda la misma XP pero sin fecha ni universidad. Arranca en cero el 14/09: lo anterior no se puede recuperar. |
| `GameDifficultyVote` (`game_difficulty_votes`) | **Una de las dos opiniones que el juego recoge, y la única que mueve algo.** Una fila por pregunta, en dos pasos: se crea al mostrarla (`answered_at` en NULL = se mostró y la ignoraron) y se completa al votar. `voto` toma los mismos tres valores que el canal A de `exercise_feedback` (`muy_facil`/`justo`/`muy_dificil`) para que los dos productos se crucen sin traducir. Guarda además **lo que el motor creía en ese momento** —`theta_at_vote`, `ventana`, `aciertos`, `p_hat_medio`— y eso no es redundancia con `game_exercises`: β se mueve con cada respuesta y `backfill_elo.py` reescribe θ de todo el historial, así que recalcularlo después daría otro número. `delta_theta` es lo que se le aplicó a θ, y cero es un valor frecuente y legítimo. `corte_ejercicio_id` (desde el 24/09) guarda **hasta dónde llegó lo que este ajuste cobró**, y es el piso de la ventana del voto siguiente: se escribe solo cuando `delta_theta` no es cero, porque lo que no se puede cobrar dos veces es lo ya cobrado. Sin esa columna, con la escalera de encuestas volviendo cada diez respuestas, dos votos seguidos se calcularían sobre media tanda compartida. NULL en todo lo anterior a esa fecha, y eso es correcto: para el corte solo importa el ajuste más reciente. |
| `GameRepetitionVote` (`game_repetition_votes`) | **La otra opinión: si le salen repetidas.** Mismo protocolo de dos pasos y misma escalera de turnos que la de arriba, con la que **alterna** (`opinion-trigger.ts`), y tres valores propios: `variado`/`justo`/`repetitivo`. `justo` es la misma palabra que en los otros dos canales del producto y significa otra cosa en cada uno, así que agrupar por valor sin filtrar por canal mezcla tres preguntas. **Tabla aparte y no un canal más de la de arriba** porque lo que congela es otro: no θ ni aciertos, sino `plantillas_distintas` y `enunciados_distintos` sobre las últimas `repetitividad.VENTANA` derivadas servidas. Los dos contadores y no uno porque ocho plantillas pueden ser ocho veces el mismo enunciado, que es el bug del 10/09. La ventana **no filtra** salteados ni mirados con la tabla, al revés que la de dificultad: mide lo que la persona vio. Con `ventana < repetitividad.MIN_VISTOS` los dos contadores quedan en cero —«no sabemos», no «vio una sola»— y el panel los deja afuera de los promedios. **Este voto no mueve θ ni XP ni nada**, y es a propósito: la ventana de exclusión del selector es una constante global, no una preferencia por persona. |
| `GameSurveyAnswer` (`game_survey_answers`) | **La pregunta abierta**, una sola vez en la vida de cada jugador, en la derivada 18. Es la única fila del sistema que puede decir algo que no se nos ocurrió preguntar: todo lo demás que el juego sabe lo sabe porque lo midió, y `GameDifficultyVote` tiene tres respuestas posibles que elegimos nosotros. Mismos dos pasos que aquella (`answered_at` en NULL = la vio y se fue). `pregunta` guarda la **clave** de lo que se preguntó (`game/encuesta.py :: VARITA`) y no el enunciado: el enunciado se retoca sin que la pregunta cambie, y el día que cambie de verdad las dos tandas quedan separadas sin migración. **Un `texto` de un punto no es basura**: la diapo no tiene botón de saltar, así que escribir «.» es cómo se dice que no, y se guarda. Qué cuenta como salto lo decide `encuesta.es_salto` **al leer** y no al escribir, para poder mover el corte sin haber perdido el dato. `correctas_al_mostrar` se congela por lo mismo que los agregados de la tabla de arriba. |
| `GameBoost` (`game_boosts`) | **Tabla compartida entre los dos productos.** Un cafecito invitado en el minijuego multiplica el XP de una universidad (o de todo Intervalo, con `university IS NULL`) por `1 + ⌈cafecitos/2⌉` horas: de 2 h por un cafecito suelto a 6 h al tope de una donación. Lo lee también el motor SM-2, vía `backend/xp_boost.py` — la única dependencia de `backend/` hacia `backend/game/`, y va en un solo sentido. |
