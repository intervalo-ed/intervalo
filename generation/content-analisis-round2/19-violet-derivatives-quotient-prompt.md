...
Esto no originó reglas nuevas (son reincidencias de las reglas críticas 4 y 15, ya existentes), pero es la evidencia concreta de que hay que auditar el 100% de los ejercicios contra esas reglas, no confiar en que sean puntuales.

**Esta ronda también agrega 3 reglas nuevas al snowball general** (originadas en otros topics de esta misma unidad, aplican igual acá): regla 30 (aligned solo para derivaciones reales), regla 31 (reintroducir la definición central en cada ejercicio) y regla 32 (sin openers cortos genéricos antes de un bloque `$$...$$`).
...
4. **Sin openers cortos y genéricos (regla 32, nueva).** Nunca abrir con `"Sabiendo que"`, `"Para derivar"` pegado directo a un bloque `$$...$$` sin cerrar la oración. La introducción es sustantiva, varía ejercicio a ejercicio, cierra en `.`/`:`, y la pregunta arranca después en mayúscula. **Confirmado como patrón dominante en este topic** (todo `RESL` abre con `"Sabiendo que"`, la mitad de `ESTR` con `"Para derivar"`).
...
1. **`backend/content/authoring-context.md` completo, sin saltear secciones.** Resumen acumulado hasta la fecha (con la regla crítica entre paréntesis):
   ...
   - Nunca enmarcar un ejercicio respecto de otro ejercicio de la sesión; todo ejercicio abstracto sitúa el concepto desde la primera oración (regla 24): aplica sobre todo a sub-D de `RESL` (datos abstractos).
   ...
   - **Regla 31 (nueva): todo ejercicio reintroduce la definición/fórmula central que usa, sin asumir que el alumno ya la vio en otro ejercicio.** En este topic aplica a la fórmula de la regla del cociente misma: si un ejercicio de `RESL` la da por conocida sin nunca haberla escrito en el propio ejercicio, conviene citarla al menos una vez en la `explanation`.
   - **Regla 32 (nueva, confirmada como patrón dominante en este topic): sin openers cortos y genéricos pegados a un bloque `$$...$$` sin cerrar la oración.** Nada de `"Sabiendo que"` ni `"Para derivar"` repetidos como plantilla en todo un skill. Variar la redacción ejercicio a ejercicio.
...
- **Corregir los ejercicios existentes** contra los hallazgos: sacar el paréntesis `"(linealidad)"` de las opciones correctas afectadas, sacar el relleno `"solamente"` de las opciones numéricas afectadas, variar/cerrar correctamente las aperturas con el patrón de plantilla corta.
...
**Paso 1 — Plan, antes de tocar un solo ejercicio.** Escribí en el chat el plan: qué corrección aplica a cada ejercicio de prueba existente por sub-familia, qué pasa con los existentes, y cómo quedaría reescrita la opción de `ESTR_04`/`ESTR_05` sin el paréntesis y una apertura de `RESL` sin el opener corto, antes de escribir el resto.
...
4. Recién con el checklist limpio y el seeder sin errores, commiteá con mensaje tipo `feat(analisis/violet/quotient): regenerar ejercicios de prueba y reglas globales de la ronda`.
