...
1. **`backend/content/authoring-context.md` completo, sin saltear secciones.** Resumen acumulado hasta la fecha (con la regla crítica entre paréntesis):
   ...
   - Nunca enmarcar un ítem respecto de otro ítem de la sesión; todo ítem abstracto sitúa el concepto desde la primera oración (regla 24): aplica a `GRAF` sub-B (propiedades de los límites).
   ...
   - Todo ítem reintroduce la definición/fórmula central que usa, sin asumir que el alumno ya la vio en otro ítem (regla 31). En este topic aplica a $\int_a^b f(x)\,dx = F(b)-F(a)$ (Barrow) misma.
   - **Openers y puntuación (regla 32).** `"Calculá"` (`RESL`) es cláusula completa: solo le falta el `:` y variar la redacción (hoy 100% idéntica en los 15 ítems).
   ...
   - Grilla 2×2: ≤35 caracteres en `RESL` y en ítems numéricos de `GRAF` (ver cardinalidad propia).
...
- **Corregir los ítems existentes** contra los hallazgos: sacar las aclaraciones entre paréntesis del `\text{}` de `GRAF#14` (mover a la prosa), ajustar la paridad de longitud de `GRAF#9`/`#12`, agregar `:` a los openers de `RESL` y variar la redacción.
...
**Paso 1 — Plan, antes de tocar un solo ítem.** Escribí en el chat el plan: qué corrección aplica a cada ítem de prueba existente por sub-familia, qué pasa con los existentes, y cómo quedaría reescrito `GRAF#14` (sin aclaraciones entre paréntesis dentro del `\text{}`) y `GRAF#9`/`#12` (opciones con paridad ajustada) antes de escribir el resto.
...
1. Corré el checklist transversal + por skill de `topic-context.md` sobre los 15 ítems de prueba por skill. Prestá atención especial a: regla dura de restricción (cero tolerancia, especialmente $+C$ prohibido en correctas), `$$...$$` con un solo `\n`, regla 26 (sin cláusulas con paréntesis dentro de `\text{}`), paridad de opciones, regla 32 (openers con `:` y redacción variada), `tags` completos y coincidentes con la tabla.
...
4. Recién con el checklist limpio y el seeder sin errores, commiteá con mensaje tipo `feat(analisis/brown/definite): regenerar ítems de prueba y correcciones de la ronda`.
