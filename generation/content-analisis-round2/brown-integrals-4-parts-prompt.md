# Prompt de refactor: analisis / brown / integrals / parts (ESTR + RESL)
...
Este topic hoy tiene ejercicios de prueba (no los 50×2=100 de la meta). En la **ronda 2** solo se **regeneran y corrigen los 15 ejercicios de prueba que ya existen** por skill, sin agregar nuevos (ver el callout de alcance arriba). Introduce el segundo método de integración (por partes).
...
- `ESTR`: 8/15 con `"Para resolver\n$$...$$\npor partes, ¿cuál es la elección correcta...?"`, que **corta una sola oración con la fórmula en el medio** (regla crítica 9).
- `ESTR`: 7/15 con `"Considerá la integral"`, cláusula completa que solo necesita el `:`.
- `RESL`: 15/15 con `"Calculá"`, mismo caso, cláusula completa que solo necesita el `:`.
...
2. **Openers y puntuación (regla 32/9, reincidencia confirmada).** `"Para resolver"` (`ESTR`) corta la oración con la fórmula en el medio (regla 9): reescribir con cierre propio antes del bloque. `"Considerá la integral"` (`ESTR`) y `"Calculá"` (`RESL`) son cláusulas completas, solo necesitan el `:` y variar la redacción.
3. **Signo negativo de la fórmula ($uv - \int v\,du$), nunca $uv + \int v\,du$.** Regla dura del topic, foco central de la explicación.
4. **$+C$ obligatorio en toda respuesta correcta y toda opción de `RESL`.**
...
1. **`backend/content/authoring-context.md` completo, sin saltear secciones.** Resumen acumulado hasta la fecha (con la regla crítica entre paréntesis):
   - Negrita solo en `question`/`explanation`, nunca en `options` (regla 1). Primera mención de conceptos clave en negrita (regla 3): en este topic, `integración por partes`, `regla LIATE`, `integrales cíclicas`, `factor oculto`.
   ...
   - "Función", nunca "regla", salvo nombre propio (regla 8). "Regla LIATE" es nombre propio válido tal cual.
   - **Nunca cortar una oración a la mitad con una fórmula display en el medio (regla 9). Reincidencia confirmada en `ESTR` (`"Para resolver"`, 8/15 ejercicios): reescribir con cierre propio.**
   ...
   - Nunca invocar conceptos fuera de la frontera del cinturón (regla 12): en este topic, nada de integral definida, Barrow, áreas, TFC, sustitución trigonométrica ni fracciones parciales (ver regla dura del topic).
   ...
   - **Un `aligned` con columna de `=` es solo para una ecuación que se despeja o una expresión que se transforma paso a paso, nunca para listar datos evaluados de forma independiente (regla 30).** En este topic, cuidado especial en la identificación de $u$, $du$, $v$, $dv$: si son valores/expresiones sueltas antes de armar $uv - \int v\,du$, van en prosa o líneas simples, no alineadas con `=` junto al cálculo real.
   - Todo ejercicio reintroduce la definición/fórmula central que usa, sin asumir que el alumno ya la vio en otro ejercicio (regla 31). En este topic aplica a $\int u\,dv = uv - \int v\,du$ misma.
   - **Openers y puntuación (regla 32, reincidencia confirmada en `ESTR` y `RESL`).** `"Considerá la integral"` (`ESTR`) y `"Calculá"` (`RESL`) son cláusulas completas: solo les falta el `:`. `"Para resolver"` (`ESTR`) es más grave, corta la oración con la fórmula (ver regla 9).
...
3. `backend/content/analisis/brown/integrals/parts/topic-context.md` — este topic. Tiene la **regla dura de restricción** (solo partes + tabla + linealidad, nada de definidas/sustitución trigonométrica/fracciones parciales, cíclicas solo diagnosticadas en `ESTR` nunca resueltas en `RESL`), la **distribución objetivo con `tags`** (ESTR 25/25, RESL 25/25), las **confusiones fuente** por skill, y **"Hallazgos de auditoría (ronda 1, jul-2026)"**.
...
Sobre `ESTR.json` y `RESL.json` de `backend/content/analisis/brown/integrals/parts/`:
...
- **Corregir los ejercicios existentes** contra los hallazgos: reescribir `"Para resolver"` sin cortar la oración con la fórmula, agregar `:` a `"Considerá la integral"` y `"Calculá"`, variar la redacción ejercicio a ejercicio en ambas skills.
- **Respetar la regla dura de restricción** del topic: solo método de partes + tabla + linealidad, nada de integral definida/Barrow/áreas/TFC/sustitución trigonométrica/fracciones parciales, cíclicas solo como diagnóstico en `ESTR` sub-B (nunca resueltas), `RESL` con una única iteración, $+C$ y signo negativo de la fórmula siempre.
...
- No relajes la regla dura de restricción del topic (nada de definidas, sustitución trigonométrica, fracciones parciales, ni cíclicas resueltas en `RESL`).

## Cómo proceder: planificar, ejecutar, commitear

**Paso 1 — Plan, antes de tocar un solo ejercicio.** Escribí en el chat el plan: qué corrección aplica a cada ejercicio de prueba existente por sub-familia, qué pasa con los existentes, y cómo quedaría reescrito un ejercicio de `ESTR` con `"Para resolver"` (sin cortar la oración) antes de escribir el resto.
...
1. Corré el checklist transversal + por skill de `topic-context.md` sobre los 15 ejercicios de prueba por skill. Prestá atención especial a: regla dura de restricción (cero tolerancia), `$$...$$` con un solo `\n`, regla 9 (sin oraciones cortadas por la fórmula), regla 32 (openers con `:` y redacción variada), signo negativo de la fórmula, $+C$ en toda respuesta y opción, `tags` completos y coincidentes con la tabla.
...
4. Recién con el checklist limpio y el seeder sin errores, commiteá con mensaje tipo `feat(analisis/brown/parts): regenerar ejercicios de prueba y correcciones de la ronda`.
