...
Este topic hoy tiene ejercicios de prueba (no los 50×2=100 de la meta). En la **ronda 2** solo se **regeneran y corrigen los 15 ejercicios de prueba que ya existen** por skill, sin agregar nuevos (ver el callout de alcance arriba). Introduce el primer método de integración (cambio de variable / $u$-sub).
...
1. **`backend/content/authoring-context.md` completo, sin saltear secciones.** Resumen acumulado hasta la fecha (con la regla crítica entre paréntesis):
   ...
   - Todo ejercicio reintroduce la definición/fórmula central que usa, sin asumir que el alumno ya la vio en otro ejercicio (regla 31). En este topic aplica a $\int f(g(x))g'(x)\,dx = \int f(u)\,du$ misma.
...
- **Corregir los ejercicios existentes** contra los hallazgos: reescribir `"Para resolver"` sin cortar la oración con la fórmula, agregar `:` a `"Calculá"`, variar la redacción ejercicio a ejercicio en ambas skills.
...
4. Recién con el checklist limpio y el seeder sin errores, commiteá con mensaje tipo `feat(analisis/brown/substitution): regenerar ejercicios de prueba y correcciones de la ronda`.
