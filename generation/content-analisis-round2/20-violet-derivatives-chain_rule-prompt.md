...
Este topic hoy tiene ítems de prueba (no los 50×2=100 de la meta). En la **ronda 2** solo se **regeneran y corrigen los 15 ítems de prueba que ya existen** por skill, sin agregar nuevos (ver el callout de alcance arriba).
...
1. **`backend/content/authoring-context.md` completo, sin saltear secciones.** Resumen acumulado hasta la fecha (con la regla crítica entre paréntesis):
   ...
   - Nunca enmarcar un ítem respecto de otro ítem de la sesión; todo ítem abstracto sitúa el concepto desde la primera oración (regla 24): aplica sobre todo a sub-C de `RESL` (datos abstractos).
   ...
   - **Regla 31 (nueva): todo ítem reintroduce la definición/fórmula central que usa, sin asumir que el alumno ya la vio en otro ítem.** En este topic aplica a la fórmula $(g \circ h)'(x) = g'(h(x)) \cdot h'(x)$ misma.
   - **Regla 32 (nueva): un imperativo con objeto concreto (`"Considerá la función"`) solo necesita el `:` antes del bloque `$$...$$`; un fragmento sin objeto propio (`"Sabiendo que"`) necesita reescribirse como cláusula completa, el `:` no lo arregla.** En ambos casos, variar la redacción ítem a ítem, no repetir la misma apertura como plantilla.
...
- **Corregir los ítems existentes** contra los hallazgos: reescribir `RESL_12` separando datos evaluados (en prosa) del cálculo real (en su propio `aligned`), reescribir `RESL_14` con fórmula/datos centrados + 2 oraciones en vez de todo inline.
...
**Paso 1 — Plan, antes de tocar un solo ítem.** Escribí en el chat el plan: qué corrección aplica a cada ítem de prueba existente por sub-familia, qué pasa con los existentes, y cómo quedaría reescrito `RESL_12` (datos en prosa, cálculo real en su propio `aligned`) y `RESL_14` (fórmula/datos centrados + 2 oraciones) antes de escribir el resto.
...
4. Recién con el checklist limpio y el seeder sin errores, commiteá con mensaje tipo `feat(analisis/violet/chain_rule): regenerar ítems de prueba y reglas globales de la ronda`.
