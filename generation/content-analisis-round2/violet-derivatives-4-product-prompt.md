...
1. **`backend/content/authoring-context.md` completo, sin saltear secciones.** Resumen acumulado hasta la fecha (con la regla crítica entre paréntesis):
   ...
   - **Regla 31 (nueva): todo ejercicio reintroduce la definición/fórmula central que usa, sin asumir que el alumno ya la vio en otro ejercicio.** En este topic aplica a la fórmula de la regla del producto misma.
...
**Paso 1 — Plan, antes de tocar un solo ejercicio.** Escribí en el chat el plan: qué corrección aplica a cada ejercicio de prueba existente por sub-familia, qué pasa con los existentes, y cómo quedaría reescrito `RESL_07` (datos en prosa, cálculo real en su propio `aligned`) y una apertura de `ESTR`/`RESL` sin el opener corto, antes de escribir el resto.
...
4. Recién con el checklist limpio y el seeder sin errores, commiteá con mensaje tipo `feat(analisis/violet/product): regenerar ejercicios de prueba y reglas globales de la ronda`.
